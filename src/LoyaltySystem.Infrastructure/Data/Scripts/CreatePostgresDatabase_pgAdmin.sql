...
BEGIN
    ...
    CREATE INDEX IF NOT EXISTS idx_store_geo_locations_location ON store_geo_locations USING GIST (location);
...
BEGIN
    ...
    DROP TRIGGER IF EXISTS update_store_geo_locations_timestamp ON store_geo_locations;
    CREATE TRIGGER update_store_geo_locations_timestamp
    BEFORE UPDATE ON store_geo_locations
    FOR EACH ROW EXECUTE FUNCTION update_timestamp();
    
    DROP TRIGGER IF EXISTS update_store_operating_hours_timestamp ON store_operating_hours;
