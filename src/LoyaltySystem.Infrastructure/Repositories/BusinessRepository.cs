...
public class BusinessRepository : IBusinessRepository
{
    ...
    public async Task<Business?> GetByIdAsync(BusinessId id)
    {
        ...
        return businessDto is { }
            ? MapFromDto(businessDto)
            : null;
    }
    ...
    public async Task<IEnumerable<Business>> SearchByNameAsync(string nameQuery, int skip = 0, int limit = 20)
    {
        const string sql = @"
            SELECT 
                b.id AS Id,
        ...
        var parameters = new { NameQuery = $"%{nameQuery}%", Limit = limit, Skip = skip };
